...
public class ImageCanvas extends Sprite {
	...
	public function addBackdropStroke(s:SVGShape):void {
		for (var i:uint = 0; i < contentLayer.numChildren; ++i) {
			var dObj:DisplayObject = contentLayer.getChildAt(i);
			if (!(dObj is SVGShape) ||
					(dObj as SVGShape).getElement().getAttribute('scratch-type') != 'backdrop-fill') {
	...
	private function getZoomLevelZero(w:uint, h:uint):Number {
		...
		return (availRatio > aspectRatio ? aspectRatio * h / canvasWidth : w / (aspectRatio * canvasHeight));
	}
	...
	public function resize(w:uint, h:uint):void {
		...
		g.drawRect(
				left - canvasBorderWidth, top - canvasBorderWidth, visibleW + 2 * canvasBorderWidth,
				visibleH + 2 * canvasBorderWidth);
		g.endFill();
	...
	public function getZoomAndScroll():Array {
		return [
			visibleArea.scaleX, hScrollbar.scrollValue(), vScrollbar.scrollValue()
		]
	}
